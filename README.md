...
[![Performance: PlayMode + Standalone (Mono)](<https://img.shields.io/badge/Performance-PlayMode%20%2B%20Standalone%20(Mono)-blueviolet.svg>)](docs/architecture/performance.md)<br/>
...
Looking for hard numbers? See per-scope (PlayMode and Standalone, Mono) dispatch and library-comparison [Performance Benchmarks](docs/architecture/performance.md).
...
For live, CI-generated per-scope dispatch tables (PlayMode + Standalone, Mono) and a cross-library comparison matrix, see [Performance Benchmarks](docs/architecture/performance.md). Those numbers are auto-generated on every push under Mono with the .NET Standard 2.1 profile and a Release build; the comparison table below is a static summary, not the live data.
