...
for (const [name, pathImpl, anchor, target, paths, expected] of [
  [
    "macOS host alias",
    path.posix,
    "/var/tmp",
    ...
    [
      ...
      ["/var/tmp/artifact", "/private/var/tmp/artifact"]
    ],
    true
  ],
  [
    ...
    [
      ["C:\\Temp", "C:\\Temp"],
      ["C:\\Temp\\Artifact", "C:\\Temp\\Artifact"]
    ],
    true
  ],
  [
    "Windows artifact alias",
    path.win32,
    "C:\\Temp",
    "C:\\Temp\\alias\\artifact",
    [
      ["C:\\Temp", "C:\\Temp"],
      ["C:\\Temp\\alias\\artifact", "D:\\private\\artifact"]
    ],
    false
  ]
]) {
  test(`isDirectDirectory handles ${name}`, () => {
    const realpathSync = fakeRealpathSync(
      new Map(paths.map(([left, right]) => [left.toLowerCase(), right]))
    );
    assert.equal(
      isDirectDirectory(target, {
        anchors: [anchor],
        caseInsensitive: pathImpl === path.win32,
