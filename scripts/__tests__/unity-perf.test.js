...
// --- render-perf-doc.js: AUTOGENERATED region heading levels (markdownlint MD001) ---
...
// Mirror markdownlint MD001 (heading-increment): walk every ATX heading and
// assert each is at most one level deeper than the previous heading. The region
// sits under "## Latest CI dispatch throughput" in performance.md, so the parent
// h2 is prepended before checking.
function firstMd001Violation(markdown) {
  let previousLevel = 0;
  let inFence = false;
  for (const line of markdown.split("\n")) {
    ...
    if (previousLevel !== 0 && level > previousLevel + 1) {
      return `"${line.trim()}" jumps from h${previousLevel} to h${level}`;
    }
    previousLevel = level;
  }
  return null;
...
test("buildDispatchSections emits h3 headings (MD001-safe under the h2 parent)", () => {
  const dispatchByScope = new Map([
    ["Standalone", new Map([["UntargetedFlood_OneHandler", dispatchRow("UntargetedFlood_OneHandler", "37690000.000")]])]
  ]);
  const sections = buildDispatchSections(dispatchByScope, ["Standalone"]);
  const heading = sections[0][0];
  assert.ok(heading.startsWith("### "), `expected h3, got: ${heading}`);
  assert.ok(!heading.startsWith("#### "), "dispatch heading must not be h4");
});

test("rendered region has no MD001 heading-increment violation under the h2 parent", () => {
  const rows = [
    dispatchRow("UntargetedFlood_OneHandler", "37690000.000"),
    {
      ...
      scenario: buildComparisonScenarioId("DxMessaging", "GlobalToOne")
    },
    {
      ...dispatchRow(buildComparisonScenarioId("MessagePipe", "GlobalToOne"), "69680000.000"),
      scenario: buildComparisonScenarioId("MessagePipe", "GlobalToOne")
    }
  ];
  const selection = selectRowsForVersion(rows, "Unity 6000.3.16f1");
  const block = buildBlock(selection, "6000.3.16f1");
  // Prepend the real parent heading the region lives under in performance.md.
  const withParent = `## Latest CI dispatch throughput\n\n${block}`;
  assert.equal(firstMd001Violation(withParent), null);
