...
test("release workflows pin App write scopes and denied-push diagnostics", () => {
  ...
  for (const [name, source, pattern] of [
    ...
    ["prepare formatting", getStepBlock(getJobBlock(prepare, "prepare"), "Validate the prepared tree"), /\n          set -euo pipefail\n[\s\S]*\n          npm run format:check\n/],
    ["prepare validation before recovery patch", prepare, /- name: Validate the prepared tree[\s\S]*- name: Push the release branch and open the PR[\s\S]*\n          recovery_dir="artifacts\/release-prepare"\n[\s\S]*git format-patch -1 --stdout/],
