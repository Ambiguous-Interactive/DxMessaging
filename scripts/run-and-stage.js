...
function stageFiles(files, options = {}) {
  ...
  process.stderr.write(
    `run-and-stage: refusing to stage stale pre-existing hunks in generated file(s). ${reverse.detail}${rollback}\n`
  );
  return 1;
}
...
function main(argv = process.argv.slice(2), deps = {}) {
  ...
  if (!indexPatch.ok) {
    process.stderr.write(
      `run-and-stage: failed to capture staged baseline: ${indexPatch.detail}\n`
    );
    return 1;
  }
  ...
  if (!unstagedPatch.ok) {
    process.stderr.write(
      `run-and-stage: failed to capture unstaged baseline: ${unstagedPatch.detail}\n`
    );
    return 1;
  }
  ...
  if (!preexistingUntracked.ok) {
    process.stderr.write(
      `run-and-stage: failed to capture untracked baseline: ${preexistingUntracked.detail}\n`
    );
    return 1;
  }
  if (!patchIsEmpty(unstagedPatch.patch)) {
    ...
    if (!indexPatch.ok) {
      process.stderr.write(
        `run-and-stage: failed to capture per-file staged baseline: ${indexPatch.detail}\n`
      );
      return 1;
    }
    ...
    if (!unstagedPatch.ok) {
      process.stderr.write(
        `run-and-stage: failed to capture per-file unstaged baseline: ${unstagedPatch.detail}\n`
      );
      return 1;
    }
  }
  ...
  const result = runFn(command[0], command.slice(1), { cwd });
  if (result.error) {
