...
namespace DxMessaging.Tests.Runtime.Core
{
    ...
    public sealed class TokenDisposeReuseTests
    {
        ...
        [Test]
        public void ReentrantSameHandlerRegistrationSurvivesOriginalDeregistration()
        {
            BusType innerBus = new();
            ReentrantReplayRemovalBus bus = new(innerBus);
            MessageHandler handler = new(new InstanceId(OwnerInstanceId), bus) { active = true };
            ...
            try
            {
                using (
                    LeakWatcher watcher = new(
                        ...
                        throwOnLeak: true,
                        label: nameof(
                            ReentrantSameHandlerRegistrationSurvivesOriginalDeregistration
                        )
                    )
                )
                {
                    MessageRegistrationHandle original =
                        token.RegisterUntargeted<SimpleUntargetedMessage>(callback);
                    Assert.IsTrue(
                        handler
                            ._handlersByTypeByMessageBus[bus.RegisteredGlobalSequentialIndex]
                            .TryGetValue<SimpleUntargetedMessage>(out object typedHandlerObject),
                        "The registration must create its typed handler."
                    );
                    MessageHandler.TypedHandler<SimpleUntargetedMessage> typedHandler =
                        (MessageHandler.TypedHandler<SimpleUntargetedMessage>)typedHandlerObject;
                    TypedSlot<SimpleUntargetedMessage> slot = typedHandler._slots[
                        TypedSlotIndex.UntargetedHandleFast
                    ];
                    MessageHandler.HandlerActionCache<MessageHandler.FastHandler<SimpleUntargetedMessage>> cache =
                        (MessageHandler.HandlerActionCache<MessageHandler.FastHandler<SimpleUntargetedMessage>>)
                            slot.byPriority[0];
                    long versionBeforeRemoval = cache.version;
                    MessageRegistrationHandle replacement = default;
                    bus.OnDeregistration = _ =>
                    {
                        bus.OnDeregistration = null;
                        replacement = token.RegisterUntargeted<SimpleUntargetedMessage>(callback);
                    };

                    token.RemoveRegistration(original);

                    Assert.AreNotEqual(
                        default(MessageRegistrationHandle),
                        replacement,
                        "The custom bus must register a replacement while deregistration is in flight."
                    );
                    Assert.AreEqual(
                        1,
                        bus.RegisteredUntargeted,
                        "Removing the original must preserve the reentrant bus registration."
                    );
                    Assert.AreEqual(
                        1,
                        token._metadata.Count,
                        "Only the reentrant replacement handle must remain staged."
                    );
                    // The bus callback increments this entry before the original
                    // teardown performs its post-bus decrement.
                    Assert.IsTrue(
                        cache.entries.TryGetValue(callback, out var replacementEntry),
                        "The replacement must remain in the typed cache."
                    );
                    Assert.AreEqual(
                        1,
                        replacementEntry.count,
                        "The replacement must retain one typed-cache reference."
                    );
                    Assert.AreEqual(
                        1,
                        slot.liveCount,
                        "The same-key replacement must keep one live typed-cache entry."
                    );
                    Assert.AreEqual(
                        versionBeforeRemoval + 2,
                        cache.version,
                        "Reentrant registration and pre-bus invalidation must each bump version."
                    );
                    ...
                    Assert.AreEqual(
                        1,
                        handled,
                        "The reentrant same-handler registration must remain dispatchable."
                    );
                    ...
                    Assert.AreEqual(
                        0,
                        bus.RegisteredUntargeted,
                        "Removing the replacement must release the final bus registration."
                    );
                    Assert.AreEqual(
                        0,
                        token._metadata.Count,
                        "Removing the replacement must clear the final token registration."
                    );
                    Assert.IsFalse(
                        cache.entries.ContainsKey(callback),
                        "Removing the replacement must clear the typed-cache entry."
                    );
                    Assert.AreEqual(
                        0,
                        slot.liveCount,
                        "Removing the replacement must clear the typed slot live count."
                    );

                    MessageRegistrationHandle recovered =
                        token.RegisterUntargeted<SimpleUntargetedMessage>(callback);
                    bus.UntargetedBroadcast(ref message);
                    Assert.AreEqual(
                        2,
                        handled,
                        "Re-registering the same key after full removal must dispatch once."
                    );
                    token.RemoveRegistration(recovered);
                    Assert.AreEqual(
                        1,
                        handler.ResetEmptyTypedSlotsForSweep(bus),
                        "The fully drained typed slot must be reclaimable."
                    );
                    Assert.IsNull(
                        typedHandler._slots[TypedSlotIndex.UntargetedHandleFast],
                        "Sweep must remove the drained typed slot."
                    );
                }
            }
            finally
