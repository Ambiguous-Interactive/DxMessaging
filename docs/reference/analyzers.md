...
- **Severity:** Warning
- **Source:** `MessageAwareComponentBaseCallAnalyzer`
- **Triggered when:** A subclass uses the `new` modifier (instead of `override`) on one of the five guarded method names, AND some ancestor actually declares a member with a matching signature. `new` on a name no ancestor declares hides nothing (compiler warning CS0109 territory) and stays silent.
...
- Non-static.
- Non-generic.
- An ancestor declares a member with a matching signature, so the declaration really is an implicit hide.
...
The unified per-FQN snapshot is persisted to `Library/DxMessaging/baseCallReport.json` so the overlay has data to render before the first post-load rescan completes; it is rewritten on every successful rescan. A manual `Tools -> Wallstop Studios -> DxMessaging -> Rescan Base-Call Warnings` menu is available for force-rescan.

The overlay itself uses two complementary editor-injection paths:

- **Package-owned component editor** (`MessageAwareComponentFallbackEditor`) -- registered as `[CustomEditor(typeof(MessageAwareComponent), editorForChildClasses: true)]`, so it owns `MessageAwareComponent` subclasses that do not have a more-specific user custom editor. Its primary path is `CreateInspectorGUI()`: it hosts the UI Toolkit warning view, then fills Unity's default Inspector body with `InspectorElement.FillDefaultInspector` so the disabled Script row and serialized fields stay identical to Unity's built-in inspector. `OnInspectorGUI()` remains as the IMGUI compatibility path and calls `RenderInsideOnInspectorGUI()` before `DrawDefaultInspector()`.
- **`Editor.finishedDefaultHeaderGUI` hook** (test seam: `TryRenderHeaderHook`) -- the compatibility path for user-defined custom editors that are more specific than the package editor. This path still renders the warning with Unity IMGUI (`EditorGUILayout.HelpBox`) because it runs inside Unity's header GUI callback, not inside a retained UI Toolkit inspector tree. The hook skips package-owned editor instances to avoid double rendering, renders only during `EventType.Repaint`, and clears its per-repaint de-dupe latch on `EventType.Layout`.

**Layout/Repaint balance.** Inside `Editor.OnInspectorGUI`, Unity invokes the editor twice per frame: once with `Event.current.type == EventType.Layout` (control registration) and once with `EventType.Repaint` (drawing). Both passes must emit _identical_ sequences of `EditorGUILayout.*` calls -- short-circuiting `OnInspectorGUI` on event type corrupts the inspector window's layout cache and breaks adjacent components. The `RenderInsideOnInspectorGUI` entry point therefore performs all "should we render?" gating up-front (before any `EditorGUILayout` call) and never gates on `EventType`. Cross-path de-dupe with the header hook is handled by an unconditional skip inside `DrawHeader` when the editor instance is the package editor -- so the package editor and header hook never both render for the same target on the same frame, regardless of Unity version.

Components that emit DXMSG006, DXMSG007, DXMSG009, or DXMSG010 show a warning surface at the top of their Inspector with three actions:

- **Open Script** -- opens the offending component's source file; when the legacy console bridge has a line number, it opens at that line.
...
The warning surface respects the per-project master toggle in `DxMessagingSettings.BaseCallCheckEnabled` on `Assets/Editor/DxMessagingSettings.asset`, exposed in **Project Settings > Wallstop Studios > DxMessaging > Inspector Checks** as **Base-Call Check Enabled**. When this toggle is off, the Inspector overlay is silenced; the underlying DXMSG006/DXMSG007/DXMSG009 compile-time warnings still emit unless you suppress them via `.editorconfig` (e.g. `dotnet_diagnostic.DXMSG006.severity = none`).
...
1. After the next compile finishes, refresh the overlay cache via `Tools -> Wallstop Studios -> DxMessaging -> Rescan Base-Call Warnings`.

For Unity 2021, the package-owned `[CustomEditor(typeof(MessageAwareComponent), editorForChildClasses: true)]` path is the primary warning surface because `Editor.finishedDefaultHeaderGUI` does not always fire for plain `MonoBehaviour` subclasses. User-defined custom editors for specific `MessageAwareComponent` subclasses still keep their body; the header hook supplies an IMGUI HelpBox for those editors when Unity fires the callback. See the [Inspector integration](#inspector-integration) section above.
