...
attempts to commit the refreshed table directly to the default branch when the
...
committed directly to the default branch after the PR merges when the
auto-commit App is provisioned and the merge-path run is still current. The
numbers therefore track the actual measured throughput of the branch under
review rather than committed state, a manually pasted table, or specific PR text.
