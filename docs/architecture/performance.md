...
The table below is regenerated by the Performance Numbers workflow
(`.github/workflows/perf-numbers.yml`) from the latest supported Unity version's
benchmark run. On a pull request the refreshed numbers are posted as a
non-blocking sticky comment; after the pull request merges, the same workflow
opens or updates an automated PR that carries the regenerated table back into
the default branch. Do not edit it by hand.
...
Latest CI benchmark run: Unity 6000.3.16f1, commit `64970aea3530f0e705dff83c4b65c8dcde3531bf`.

Platform: Editor Mono x64 Development (WindowsEditor; Unity 6000.3.16f1).

| Scenario                                    | Throughput / Wall clock | Allocated bytes |
| ------------------------------------------- | ----------------------- | --------------- |
| UntargetedFlood_OneHandler                  | 12.07 M emits/sec       | 0 B             |
| UntargetedFlood_FourHandlers_OnePriority    | 2.68 M emits/sec        | 0 B             |
| UntargetedFlood_FourHandlers_FourPriorities | 3.01 M emits/sec        | 0 B             |
| TargetedFlood_OneListener                   | 9.54 M emits/sec        | 0 B             |
| TargetedFlood_SixteenListeners              | 0.55 M emits/sec        | 0 B             |
| BroadcastFlood_OneHandler                   | 6.48 M emits/sec        | 0 B             |
| InterceptorHeavy_FourInterceptors           | 2.42 M emits/sec        | 0 B             |
| PostProcessingHeavy_FourPostProcessors      | 2.11 M emits/sec        | 0 B             |
| RegistrationFlood_1000Types_FromColdBus     | 19.554 ms               | 0 B             |
...
refreshed by an automated PR after the PR merges. The numbers therefore track
the actual measured
