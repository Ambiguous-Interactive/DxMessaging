...
The external comparison packages are installed from the single source `.github/comparison-packages.json` (registry scopes + PINNED versions). `run-ci-tests.ps1 -IncludeComparisons` injects that registry and those pins into the ephemeral comparison manifest; the committed `.unity-test-project/Packages/manifest.json` mirrors the pins for local parity. Bump versions in the JSON only; see [Comparison Parity and Package Single Source](../testing/comparison-parity-and-package-single-source.md).
