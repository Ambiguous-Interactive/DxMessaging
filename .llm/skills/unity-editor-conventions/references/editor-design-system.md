...
- **A detached visual tree swallows every interaction.** UI Toolkit dispatches `ChangeEvent` and `ClickEvent` through the panel, so setting `Toggle.value` or sending a `ClickEvent` on a tree that was never added to a shown window's `rootVisualElement` raises no callback and the assertion fails with no hint why. Build interaction tests into a host window; structure-only assertions can stay detached.
- **Wire buttons with `RegisterCallback<ClickEvent>`, not the `Button(Action)` constructor.** A synthesized `ClickEvent` does not invoke `Button.clicked` (probed directly on Unity 6000.4, attached or not), because `Clickable` activates on pointer events. Every package button already follows this, which is why `SendClick` works on them. `NavigationSubmitEvent` also activates a button but is 2022.1+, so it breaks the 2021.3 floor.
