...
- **Skill files about the policy.** Files under `.llm/skills/documentation-style/`, `.llm/skills/markdown-authoring/`, and `.llm/skills/documentation-code-samples/` are wholly exempt.
...
- **Auto-generated files.** `llms.txt` is exempt because it is regenerated mechanically by `scripts/update-llms-txt.js`. (`.llm/index.md` is generated wholesale from each skill's own description, so the policy applies to the `description` fields it is built from.)
...
- [Code Samples Must Compile](../../documentation-code-samples/references/code-samples-must-compile.md)
