...
- Comparison and dispatch tables measure different shapes on purpose. Only `GlobalToOne` is a true topology twin of `UntargetedFlood_OneHandler`. `StructNoBox` shares its storage shape but dispatches the canonical `ComparisonStructPayload`; do not force the others equal. `ComparisonDispatchTopologyTests` pins the relationship against the methodology runbook table.
