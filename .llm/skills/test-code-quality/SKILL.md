...
- The harness asserts one result per (tech, scenario) pair so a dedup or a missing registration fails loudly. `ComparisonHarness.Run` builds and disposes a fresh bridge per case and a fresh `MessageBus` per scenario, so a divergence is always topology, never leaked state.
- Comparison and dispatch tables measure different shapes on purpose. Only `GlobalToOne` and `StructNoBox` are topology twins of `UntargetedFlood_OneHandler`. Do not force the others equal; `ComparisonDispatchTopologyTests` pins the relationship against the methodology runbook table.
