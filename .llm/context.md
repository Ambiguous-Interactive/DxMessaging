...
- Read the skill catalog first: [Skill Index](./index.md)
- Prefer focused skills over adding large instruction blocks here.
- Keep this file under 300 lines at all times.

Skills follow the [Agent Skills](https://agentskills.io/specification) open standard. Each lives at
`.llm/skills/<name>/SKILL.md` with supporting detail in a sibling `references/` directory, and is
mirrored into `.claude/skills/` and `.agents/skills/` so Claude Code, Codex, and GitHub Copilot all
discover it. `.llm/` is the single source of truth; the mirrors are generated pointers.

<!-- BEGIN GENERATED SKILL REGISTRY -->

<!-- Generated by `npm run llm:index`. Do not edit by hand. -->

34 skills are registered. See [the skill index](./index.md) for the full table.

`allocation-and-leak-coverage`, `api-design-patterns`, `benchmark-methodology`, `cache-eviction`, `changelog-management`, `collection-pooling`, `data-driven-tests`, `devcontainer-ai-backends`, `dispatch-hot-path`, `documentation-code-samples`, `documentation-style`, `github-access`, `github-workflow-consistency`, `il2cpp-build-configuration`, `markdown-authoring`, `memory-reclamation`, `object-pooling`, `package-publishing`, `plan-maintenance`, `pull-request-writing`, `serializable-dictionary`, `shell-and-powershell`, `singleton-patterns`, `test-code-quality`, `test-coverage-design`, `test-diagnostics`, `test-failure-investigation`, `test-fixtures-and-cleanup`, `unity-editor-ci`, `unity-editor-conventions`, `unity-licensing`, `unity-mcp-test-loop`, `unity-test-execution`, `value-equality-and-hashing`
...
- Keep `PLAN.md` as a transient routing page for in-progress and future work, below 100 lines at all
  times. It is a ceiling, not a target.
- Never store completed work, session narratives, copied issue checklists, research notes,
  benchmarks, or protocol detail in `PLAN.md`.
- Move completed work and evidence to `progress/session-*.md` and the relevant GitHub issue. Move
  reusable rules and protocols to a focused skill, context file, or runbook.
- Follow [plan-maintenance](./skills/plan-maintenance/SKILL.md) whenever creating, reviewing,
  updating, or resuming work from `PLAN.md`.
...
- Mirror the shipped version's CHANGELOG section into `package.json` `_upm.changelog`, which the
  Unity Package Manager renders in Version History: `npm run sync:upm-changelog` (check-only:
  `npm run check:upm-changelog`, gated in `validate:all`, regenerated by `release-prepare.yml`
...
[Unity MCP Test Loop](./skills/unity-mcp-test-loop/references/mcp-test-loop.md) for the full loop.
...
- Unity versions are single-sourced in `.github/unity-versions.json` (`all` = full CI set, `latest` = last `all` entry, `release` = pinned release version); bump ONLY that file. `npm run validate:unity-versions` enforces zero drift. See [Unity Version Single Source of Truth](./skills/unity-editor-ci/references/unity-version-single-source.md).
...
- `scripts/unity/run-ci-tests.ps1 -ProjectPath` is treated as an ephemeral CI project path: repo-contained values must live under `.artifacts/u/` (the local default), `.artifacts/unity/projects/`, or `.artifacts/unity/game-ci-projects/`, and existing external directories must carry `.dxmessaging-ci-project` before the harness will reuse them. Active CI passes runner-local external project and cache paths explicitly.
...
Follow [GitHub Access](./skills/github-access/SKILL.md). Its access order is mandatory: use the
VS Code/Codex connector or GitHub extension first, local `git` second, and `gh` only as the final
fallback. A failed `gh auth status` is not a blocker while either earlier path works.
...
- Tests for memory holders keyed by message type or `InstanceId` must prove forced trim, idle sweep, slot-count recovery, and stale deregistration behavior; see [Memory Reclaim Coverage](./skills/memory-reclamation/references/memory-reclaim-coverage.md).
- Benchmark and performance/allocation tests stay isolated under `Tests/Runtime/Benchmarks` in asmdef `WallstopStudios.DxMessaging.Tests.00.Runtime.Benchmarks`; keep `BenchmarkAssemblyContractTests` green when adding or moving perf tests. Benchmarks warm up then measure ONE continuous window; the published headline is Standalone IL2CPP (Release player, Release C++ config), .NET Standard 2.1 + Release code optimization; PlayMode/EditMode stay as local/CI test scopes, not published numbers. See [Benchmark Methodology: Total Over One Window](./skills/benchmark-methodology/references/benchmark-methodology-total-over-window.md) and [Perf Config: IL2CPP Release, .NET Standard 2.1](./skills/il2cpp-build-configuration/references/perf-config-il2cpp-release-netstandard21.md).
- When adding a `MessageCache<>` storage field to `MessageBus`, update `MessageBus.ExpectedMessageCacheFieldCount`, add the field to `MessageBus.SweepableTypeCaches`, and add reclamation coverage; see [DxMessaging Memory Reclamation](./skills/memory-reclamation/references/memory-reclamation.md).
...
- Every C# code sample in docs - inline, fenced, template-embedded, and XML `<code>` blocks - must compile; see [Code Samples Must Compile](./skills/documentation-code-samples/references/code-samples-must-compile.md) and keep `.docs-tests` green.
- Documentation prose must avoid LLM-style filler, marketing adjectives, hedge transitions, and vague quantifiers; see [Human-Prose Documentation Policy](./skills/documentation-style/references/human-prose-policy.md).
- Subclasses of `MessageAwareComponent` MUST call `base.<method>()` from every guarded lifecycle override; see [MessageAwareComponent Base-Call Contract](./skills/unity-editor-conventions/references/base-call-contract.md).
- When editing `Runtime/Core/Configuration/DxMessagingRuntimeSettings.cs` or its provider, update `docs/reference/runtime-settings.md` and `docs/guides/memory-reclamation.md` in the same change; see [Memory Reclamation Documentation Maintenance](./skills/memory-reclamation/references/memory-reclamation-docs.md).

## Skill Maintenance

- A skill is a directory: `.llm/skills/<name>/SKILL.md` plus an optional `references/` directory.
- `SKILL.md` frontmatter must satisfy the Agent Skills spec: `name` (lowercase, hyphen-separated, matching the directory) and `description` are required. Repository-specific fields belong under `metadata`; unknown top-level keys are rejected.
- The `description` is the only text an agent sees before activating a skill, so it must say both what the skill covers and when to reach for it, using the words someone would actually type.
- `SKILL.md` targets 150 lines and fails above 200. It is a router: state the rules that must always hold, then link the reference that carries the detail. Reference files fail above 500 lines.
- Run `npm run llm:index` after adding, renaming, or removing a skill; it regenerates `.llm/index.md`, the registry block above, and both agent mirrors. `npm run llm:check` gates drift in `validate:all`.
- Never hand-edit `.llm/index.md` or a generated mirror under `.claude/skills/**` or `.agents/skills/**`. The generator reaps only mirror files carrying its `Generated by npm run llm:index` marker, so a hand-authored `.claude/skills/<name>/SKILL.md` of your own is left alone by both `llm:index` and `llm:check`.
...
- Both mirror roots are kept on purpose. Claude Code discovers only `.claude/skills/`, Codex discovers only `.agents/skills/`, and VS Code Copilot reads both plus `.github/skills/`, so Copilot sees every skill twice. That duplication follows from the standard's discovery rules; publishing to one root only would de-duplicate Copilot while breaking Claude Code or Codex, so it is accepted rather than collapsed.

## See Also

- [Skill Index](./index.md) - every skill, its description, and its reference count.
- [Unity MCP setup](../scripts/mcp/README.md) - bridge, discovery, and client configuration.
