...
- Run `npm run llm:index` after adding, renaming, or removing a skill; it regenerates `.llm/index.md`, `.llm/index.json`, the registry block above, and both agent mirrors. `npm run llm:check` gates drift in `validate:all`.
- Never hand-edit `.llm/index.md`, `.llm/index.json`, or a generated mirror under `.claude/skills/**` or `.agents/skills/**`. The generator reaps only mirror files carrying its `Generated by npm run llm:index` marker, so a hand-authored `.claude/skills/<name>/SKILL.md` of your own is left alone by both `llm:index` and `llm:check`.
