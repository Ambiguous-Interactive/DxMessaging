...
Skills follow the [Agent Skills](https://agentskills.io/specification) open standard. Each lives only
in `.llm/skills/<name>/SKILL.md`, with supporting detail in a sibling `references/` directory.
Agent entry points route here and then to the skill index. Do not create client-specific skill
copies, pointers, or symlinks. Native client skill menus may not list repository skills; use the
index to select and read the canonical instructions.
...
- Regenerate the skill index and registry block: `npm run llm:index` (check-only: `npm run llm:check`, gated in `validate:all`)
