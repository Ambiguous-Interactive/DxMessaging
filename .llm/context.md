...
- Before declaring a task done, committing, or pushing, every agent/shell should run the provider-neutral `npm run preflight`; Claude adapters also run it automatically at edit/git boundaries. `npm run preflight` inspects exactly what you changed (committed-vs-integration-base + staged + unstaged + untracked, or an explicit `--range-from/--range-to` pushed range), delegates file -> hook selection to pre-commit itself (`--from-ref/--to-ref` + `--files`), self-heals `node_modules` / `pre-commit` first, and runs the relevant lint/spelling/doc/changelog/YAML/policy checks in-loop. A PreToolUse git-boundary guard first runs changed-file cspell over the full change-set, then runs full-scope preflight and blocks likely `git commit` / `git push` commands on failure; commit commands are scoped to the pre-commit stage for speed. The direct cspell pass prevents already-committed, generated, or shell-written spelling failures from reaching native pre-push when the broader guard preflight is slow. An advisory Stop hook runs preflight in working-tree scope when you end a turn. A PostToolUse guard (`scripts/hooks/post-edit-validate-guard.js`) runs the fast validators relevant to each edited file (file-scoped cspell for `.md` / `.markdown` / `.cs` / `.json` / `.yml` / `.yaml` / `.ps1` / `.js`; changelog coverage when you touch likely user-visible `Runtime/` / `Editor/` / `SourceGenerators/` / `Samples~/` files or `CHANGELOG.md`; npm-packaging when you touch `package.json` / `.npmignore` / `*.dll` / `*.meta`; ASCII/prose/code-sample checks for `.md` / `.cs`) so those failures surface at edit time instead of waiting for the native hook. For full parity on demand run `npm run preflight:pre-push`. The full file-glob -> validator dispatch table (with per-failure remediation, e.g. `W002` guidance), the flags, the Node-direct fallback, and the loud yamllint-skip live in [Change-Aware Preflight](./skills/scripting/change-aware-preflight.md).
- Native Git hooks are versioned under `scripts/hooks` and installed by `scripts/install-git-hooks.js` through `core.hooksPath`; only commit/push boundary hooks are required (`pre-commit`, `pre-push`). Do not add post-checkout/post-merge/post-rewrite mutating hooks; ordinary git operations must not dirty the tree or bootstrap pre-commit. The `pre-commit` hook repairs Node tooling, auto-installs pinned `pre-commit`, and delegates to the pre-commit framework. After the PreToolUse commit guard passes it writes a pre-commit stamp covering HEAD, the index tree, and pathspec-limited unstaged/untracked changelog-relevant content so the native pre-commit wrapper can skip the repeated run in under a second only when the validated commit/changelog surface is unchanged. The `pre-push` hook delegates to `scripts/run-native-prepush.js`, reads Git pre-push stdin, skips deleted/no-op ref updates before bootstrapping tooling, validates existing refs with exact `remote_oid..local_oid` ranges, validates new refs from the default-branch merge-base, strips caller-provided `SKIP`, and calls `scripts/preflight.js --stage=pre-push --profile=guard --range-from <from> --range-to <to> --no-worktree`. After a clean full-scope or explicit-range `npm run preflight` with no caller-provided `SKIP`, a pre-push stamp may skip native range validation only when Git's exact pushed range matches the validated range exactly; stale, missing, narrower, broader, skipped, diverged, multi-tip, or no-base pushes still run native validation. Native local push must not run `--all-files`, `npm run doctor`, or `scripts/run-prepush-parallel.js`; if no merge-base exists for a new ref it falls back to exhaustive `npm run preflight:pre-push`. Exhaustive parity belongs to CI/manual `npm run preflight:pre-push`; `scripts/run-prepush-parallel.js` is retained only as an explicit exhaustive accelerator with coverage equivalence pinned by `scripts/__tests__/run-prepush-parallel.test.js`. The git hook is the last-resort local backstop, not the first signal -- see [Native Git Hook Bootstrap](./skills/scripting/native-git-hooks.md).
...
- `package.json` is the committed source of truth (EXACT pins); `package-lock.json` is gitignored (per-machine, regenerated). After any dependency pin change -- yours, a `git pull`, or a Dependabot bump -- the local lockfile + `node_modules` can go stale, and `npm ci`-based repair RE-CEMENTS the stale version because it honors the local lockfile. The fix is always `npm install`, never `npm ci`. `scripts/lib/dependency-version-parity.js` enforces installed == lockfile == declared for exact pins (range pins must be satisfied) offline; `scripts/repair-node-tooling.js` reconciles via `npm install` before the npm-ci gate with zero manual touch. Edit-time, the post-edit guard runs `npm run validate:dependency-parity` on `package.json` / `package-lock.json` edits, and `validate:node-tooling` + the doctor's `node_modules freshness` section + `scripts/__tests__/dependency-version-parity.test.js` gate it. Do NOT one-side a pin (e.g. bump `cspell` without `cspell-lib`); see [Dependency Version Parity](./skills/scripting/dependency-version-parity.md).
...
- Unity versions are single-sourced in `.github/unity-versions.json` (`all` = full CI set, `latest` = last `all` entry, `release` = pinned release version); bump ONLY that file. `scripts/validate-unity-versions.js` (`npm run validate:unity-versions`) enforces zero drift -- jq-reading workflows carry no literals, `mirror-all`/`mirror-release` consumers keep validated literals -- wired into actionlint CI, preflight, and native pre-push. See [Unity Version Single Source of Truth](./skills/github-actions/unity-version-single-source.md).
...
- `.devcontainer/cache-contract.sh` derives `CACHE_WORKSPACE_ROOT` from the script's own location (parent of `.devcontainer/`, equals `${containerWorkspaceFolder}`) when `WORKSPACE_FOLDER` is unset (e.g. during `postCreateCommand`); never hardcode an absolute fallback. Enforced by `scripts/__tests__/devcontainer-cache-contract.test.js`.
...
- Contract tests asserting on the CONTENT of XML / structured config files (`.props`/`.csproj`/`.targets`/`.sln`) MUST be formatting-invariant: parse them through `scripts/lib/msbuild-xml.js` (`getPropertyValue` / `resolveProperty` / `hasElement`). Never hand-roll regex/substring assertions that assume single-line tags or a fixed attribute order -- the repo's Prettier does not format XML (there is no `@prettier/plugin-xml`), so editors and future formatters can legitimately re-wrap a closing `>` onto its own line or split a multi-attribute open tag one attribute per line without changing semantics. This is enforced repo-wide by `scripts/__tests__/xml-contract-assertion-policy.test.js` and surfaced in-loop while editing by `scripts/hooks/post-edit-validate-guard.js` (its `xml-build-contract` entry runs `scripts/validate-analyzer-build-contract.js`, whose checks live in `scripts/lib/analyzer-build-contract.js`).
